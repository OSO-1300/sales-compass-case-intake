...
顧客情報、案件情報、受付票画像、`customers`、`cases`、`incoming`内の運用データ、社員カルテ、内部設計書は含めません。
...
6. 必要に応じて制作・対応依頼、AI指示、回答期限、緊急度、添付資料を入力する
7. 「受付票を表示」を押す
8. 表示された受付票をスクリーンショットする
9. スクリーンショット画像と添付資料をSales Compassのincomingへ渡す

必要に応じて「LEONへ引き継ぐ」を押すと、後段のLEON処理へ渡す受付データを端末へ保存できます。

## incoming

`incoming/`はLEONが案件を最初に受け取る受信トレイです。

iPhoneで保存先を選択するときは、Sales Compass直下の`incoming/`を選びます。

このリポジトリでは`incoming/README.md`だけを管理し、受付データや添付資料はコミットしません。
...
1. iPhone Safariの受付票から「LEONへ引き継ぐ」を実行する
2. ローカル環境でCLIを実行する
...
`production_requests`と`ai_instructions`は、LEONおよびAI社員への補足依頼として扱います。
Phase2のCustomer/Case管理基盤では保存フォルダ生成を優先するため、この2項目の部署振り分けや制作依頼化はまだ行いません。
